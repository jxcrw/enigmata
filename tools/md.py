#!/usr/bin/env python3
"""Tools for manipulating Markdown"""

import re


def find_table(markdown: str) -> str:
    """Find the first table in Markdown text."""
    pattern = r'\|(.*)\|'
    table = re.search(pattern, markdown, re.DOTALL)[0]
    return table


def add_table_row(cols: list[str], table: str) -> str:
    """Cleanly add row to Markdown table and return resulting table."""
    row = '|'.join([''] + cols + [''])
    table = table + '\n' + row
...
# ┌─────────────────────────────────────────────────────────────────────────────
# │ Format Markdown Tables (https://github.com/bitwiser73/MarkdownTableFormatter)
# └─────────────────────────────────────────────────────────────────────────────
...
def format_table(raw_table, margin=1, padding=0, default_justify=Justify.LEFT):
